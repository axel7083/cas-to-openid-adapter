...
func (s *Storage) ValidateRequest(user *User, id string) error {
	s.lock.Lock()
	defer s.lock.Unlock()
...
func (s *Storage) SigningKey(ctx context.Context) (op.SigningKey, error) {
	// in this example the signing keys is a static rsa.PrivateKey and the algorithm used is RS256
	// you would obviously have a more complex implementation and store / retrieve the keys from your database as well
...
func (s *Storage) KeySet(ctx context.Context) ([]op.Key, error) {
	// as mentioned above, this example only has a single signing keys without keys rotation,
	// so it will directly use its public keys
	//
	// when using keys rotation you typically would store the public keys alongside the private keys in your database
	// and give both of them an expiration date, with the public keys having a longer lifetime
...
func (s *Storage) ValidateTokenExchangeRequest(ctx context.Context, request op.TokenExchangeRequest) error {
	print("ValidateTokenExchangeRequest\n")
...
func (s *Storage) SetUserinfoFromTokenExchangeRequest(ctx context.Context, userinfo *oidc.UserInfo, request op.TokenExchangeRequest) error {
	print("SetUserinfoFromTokenExchangeRequest\n")
...
func (s *Storage) getTokenExchangeClaims(ctx context.Context, request op.TokenExchangeRequest) (claims map[string]interface{}) {
	...
	claims = appendClaim(claims, "email", user.Email)
